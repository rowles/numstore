...
std::shared_ptr<unsigned char> mmap_ptr(void *addr, size_t length, int prot,
                                        int flags, int fd,
                                        off_t offset) noexcept {
  auto deleter = [length](void *p) { munmap(p, length); };

  return {
      static_cast<unsigned char *>(mmap(addr, length, prot, flags, fd, offset)),
      deleter};
}
...
void mmap_file::open() {
  ...
  addr = mmap_ptr(nullptr, len, prot, MAP_SHARED, fd, 0u);
  if (addr == nullptr) {
...
std::shared_ptr<unsigned char> mmap_file::address() const { return addr; }
